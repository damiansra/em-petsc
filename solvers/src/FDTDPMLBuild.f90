...
contains
  ...
  subroutine build_pml(pml,num_pml,pml_type,ddx,ddy,dt,norder,Ro,co,xi,xf,yi,yf,nx,ny)

    implicit none
    integer, intent(in) :: num_pml,pml_type,norder
    integer, intent(in) :: xi, xf, yi, yf, nx, ny
    double precision, intent(in) :: ddx, ddy,dt,Ro,co
    double precision, dimension(num_pml,xi:xf,yi:yf), intent(inout) :: pml
    double precision, dimension(num_pml) :: sigex, sigmx, sigey, sigmy, cax, cbx, cay, cby, dax, dbx, day, dby
    double precision :: sigmex, sigmmx, sigmey, sigmmy, rex, rmx, rey, rmy
