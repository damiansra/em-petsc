...
class Params(object):
    ...
    dimension = np.zeros([num_dim])                 # create arrays
    dimension_lower = np.zeros([2])
    dimension_upper = np.zeros([2])
    ...
    def set_dims(self,shape=aux_shape):
        ...
        self.__ddt = 0.90/(self.co*np.sqrt(1.0/(self.dimension_steps[0]**2) + 1.0/(self.dimension_steps[1]**2)))
        self.dt = self.__ddt
        return self
